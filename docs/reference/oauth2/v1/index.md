...
OAuth2.0では、作成したアプリケーションごとにクライアント必要です。クライアントの発行は、[認証情報](https://manager.dmdata.jp/control/dev/credentials)よりできます。
...
**認可エンドポイント**に認可リクエストを送信し、ユーザーに認可されると、DMDATA.JPは登録されたリダイレクトURIに認可コードを送信します。
