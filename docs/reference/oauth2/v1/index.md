...
しかし、ローカル環境では使用できるポートが不定であるということから、下記に全て一致するリダイレクトURIついてはポートのチェックをしません。

* スキーム名が `http` または `https` であること。
...
|redirect_uri|はい|**String** <br/> 認可コードを送信するURIで、OAuth クライアントで指定されたリダイレクトURIを厳密に一致させる必要があります。<br/>なお、指定されたリダイレクトURIが1個のみの場合は必須ではありません。|
...
|redirect_uri|オプション|**String** <br/> 認可コード要求する際にパラメータに入力した`redirect_uri`を指定します。|
