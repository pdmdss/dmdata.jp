...
気象庁から送られてくるXMLデータのほか、DMDATA.JPで独自で定めたJSONに変換したデータも配信します。
...
* アプリケーション等で外部へ配布する際は、一般の方向けへの注意文と同義となる文章を必ず明記してください。
...
個人向けのプランを契約で、当配信サービスから受け取った緊急地震速報（予報）の内容を、広く公開するAPI等の提供は禁止（データを加工したものも含みます）させていただいておりますが、以下の図にある制限内での利用はできます。
...
* 2026年04月01日 文章の一部を変更 - ポリシーの運用は変わっていません。
...
DMDATA.JP内での処理時間平均（電文を受け取ってから利用者へ配信が開始できる状態になるまで）はXML電文で10ms、JSONに変換した電文で15～70msです。
