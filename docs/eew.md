...
気象庁が発表（送信）した順番でDMDATA.JPから配信しますが、2023年04月01日現在下の様な配信順序（表の上から順番に配信）となっています。
...
なお、この配信順序は表作成時点のものであり保証するものではありません。配信順序は予告なく変更される場合があります。
...
個人向けのプランを契約で、当配信サービスから受け取った緊急地震速報（予報）の内容を、広く公開するAPI等の提供は禁止させていただきます。

広く公開するAPIは、クローズド（アプリケーション専用）で利用するクライアントが大規模（おおよそ50以上）、
または第三者がAPIのエンドポイントや仕様等を参照できるAPIとしますが、 
要件により判断します。

また、当配信サービスから受け取った緊急地震速報の内容を第三者へ表示・報知等に使用する場合は、製作者が当サービスに事前の許可を得てから行ってください。
内容によりお断りさせていただく場合があります。

**法人契約（法人・団体アカウント、契約区分の後に（法人）とついているもの）の場合はこのポリシーに関係なくご利用いただけます。二次利用について連絡も不要です。**
